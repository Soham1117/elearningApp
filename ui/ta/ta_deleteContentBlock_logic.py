...
from ui.ta.ta_deleteContentBlock_window import Ui_TaDeleteContentBlockWindow

class TADeleteContentBlockLogic(QtWidgets.QWidget):
    def __init__(self, args):
        super().__init__()
        self.ui = Ui_TaDeleteContentBlockWindow()
        ...
        self.ui.pushButton.clicked.connect(self.handle_delete_content_block)
    ...
    def handle_delete_content_block(self):
        ...
        if content_block_id[:5] != "Block":
            ...
            return
        response,error=self.user_dao.delete_content_block(self.textbook_id,self.chapter_id,self.section_id, content_block_id)
        if response:
